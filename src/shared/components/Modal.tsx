...
function Modal() {
  ...
  return (
    <article className="flex justify-center items-center bg-black/60 fixed top-0 left-0 min-h-screen w-full z-50 p-10">
      <div className="flex flex-col rounded-2xl bg-white p-4 w-full md:w-1/2 max-w-4xl">
        <div className="flex justify-center p-5 py-3 gap-10 items-start">
          ...
          <div className="flex-1">
            <h1 className="text-secondary-700 font-bold font-inter">{text}</h1>
            <p>{message}</p>
          </div>
        </div>
        <HorizontalLine />
        <div className="flex justify-end items-center p-4">
          <div className="flex gap-4">
            ...
            <Button
              styles={`${bgColor} text-white md:text-md w-auto px-7`}
              onClick={fn}
              aria-label={text}
            >
              {text}
